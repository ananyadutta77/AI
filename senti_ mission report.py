...
print(f"Type {Fore.YELLOW}'reset'{Fore.CYAN} to reset, {Fore.YELLOW}'history'{Fore.CYAN} to view conversation history, {Fore.YELLOW}'report'{Fore.CYAN} to generate mood report, or {Fore.YELLOW}'exit'{Fore.CYAN} to quit.{Style.RESET_ALL}\n")
...
while True:
    ...
    #mission report
        ...
        if not convo_history:
            ...
            #new built infunction to be learnt
            t_polarity=sum(polarity for __, polarity, __ in convo_history)
            avg_polarity=t_polarity/len(convo_history)
            if avg_polarity>0.25:
                    ...
                    color=Fore.YELLOW
                    emoji="🙃"    
            print(f"{idx}. {color}{emoji} Your overall mood is {overall_mood}!"
                      f"(Polarity: {avg_polarity:.2f}){Style.RESET_ALL}")
